...
export default function App() {
            ...
            socket.onmessage = async (event) => {
                if (event.data instanceof Blob) {
                    try {
                        ...
                        console.log("Audio data added to queue. Queue length:", audioQueue.length);
                        if (!isPlaying) {
                            playNextInQueue();
                        }
                    }
    ...
    const startRecording = async () => {
        try {
            ...
            let silenceDebounceTimeout = null;
            const silenceDebounceDelay = 300; // 300ms debounce delay
            workletNode.port.onmessage = (e) => {
                const float32Array = new Float32Array(e.data);
                const rms = Math.sqrt(float32Array.reduce((sum, sample) => sum + sample * sample, 0) /
                    float32Array.length);
    ...
    async function resampleAudio(audioData, fromSampleRate, toSampleRate) {
        ...
        if (fromSampleRate === toSampleRate) {
            ...
            return Float32Array.from(audioData, (sample) => sample / (sample >= 0 ? 0x7fff : 0x8000));
        }
        ...
        for (let i = 0; i < newLength; i++) {
            ...
            const upperIndex = Math.min(Math.ceil(originalIndex), audioData.length - 1);
    ...
    async function playNextInQueue() {
        ...
        try {
            ...
            const resampledData = await resampleAudio(audioData, 24000, audioContext.sampleRate);
            // Added debug logs to verify buffer creation
            // Create buffer with correct sample rate for agent's audio
            const buffer = audioContext.createBuffer(1, resampledData.length, audioContext.sampleRate);
            ...
            console.log("AudioBufferSourceNode connected to GainNode and destination");
            source.onended = () => {
...
const convertFloatToPcm = (float32) => {
